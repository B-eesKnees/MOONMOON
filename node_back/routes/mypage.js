  ...
  `;
    ...
    db.query(userQuery, [userEmail], (userError, userResults) => {
      ...
      `;
            ...
            db.query(
                gradeCouponQuery,
                [userTotalPaymentInPeriod, userTotalPaymentInPeriod, userTotalPaymentInPeriod, userTotalPaymentInPeriod, userTotalPaymentInPeriod, userTotalPaymentInPeriod],
              ...
              `;
                        ...
                        db.query(existingCouponQuery, [userEmail, gradeCouponsToGive[0].COUPON_GRADE], (existingCouponError, existingCouponResults) => {
                  ...
                  `;
                                ...
                                const insertPromises = gradeCouponsToGive.map((coupon) => {
                                    return new Promise((resolve, reject) => {
                                        db.query(cpuserInsertQuery, [userEmail, coupon.COUPON_ID, 0, coupon.COUPON_MAXDATE], (insertError, insertResults) => {
                                            if (insertError) {
...
router.get("/orderpaydate", (req, res) => {
    ...
    const query = "SELECT ORDER_USER_EMAIL, DATE_FORMAT(ORDER_PAYDATE, '%m-%d') AS ORDER_PAYDATE FROM `order` WHERE ORDER_USER_EMAIL = ?";
...
router.put("/updatebuycheck/:orderItemId", (req, res) => {
    ...
    const selectQuery = "SELECT o.ORDER_STATE, oi.ORDERITEM_BUYCHECK, o.ORDER_USER_EMAIL FROM `order` o JOIN orderitem oi ON o.ORDER_ID = oi.ORDERITEM_ORDERID WHERE oi.ORDERITEM_ID = ? ";
    db.query(selectQuery, [orderItemId], (err, results) => {
        ...
        if (orderState !== "배송완료") {
            res.status(400).json({ error: "배송완료 상태가 아닙니다. 구매확정 할 수 없습니다." });
            return;
        }
        ...
        const updateQuery = "UPDATE orderitem SET ORDERITEM_BUYCHECK = 1, ORDERITEM_CONFIRMED_AT = NOW() WHERE ORDERITEM_ID = ?";
